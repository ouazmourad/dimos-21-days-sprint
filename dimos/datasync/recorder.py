...
class StandaloneRecorder:
    """Lightweight recorder that works without the DimOS Module/Blueprint system.

    Use this for scripts and demos. For blueprint integration, use DataRecorderModule.
    """
    ...
    def start(self) -> None:
        lcm = LCM()
        lcm.start()
        unsub = lcm.subscribe_all(self._on_message)
        self._unsubs.append(unsub)
        self._unsubs.append(lcm.stop)
        logger.info("StandaloneRecorder started", session_id=self._session.session_id)
    ...
    def _on_message(self, msg: Any, topic: Any) -> None:
        ...
        if self._topic_filter is not None:
            ...
            if not self._topic_filter.search(topic_name):
                return
        if self._max_rate_hz > 0:
            now = time.monotonic()
            last = self._last_record.get(topic_key, 0.0)
            if now - last < 1.0 / self._max_rate_hz:
                return
    ...
    def stop(self) -> None:
        ...
        self._unsubs.clear()
        self._session.close()
        logger.info("StandaloneRecorder stopped",
                     session_id=self._session.session_id,
                     messages_recorded=self._msg_count)
